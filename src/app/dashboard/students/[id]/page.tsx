...
export default async function StudentDetailPage({
  ...
  const student = await Student.findOne({ _id: id, schoolId: user.schoolId }).lean()
  if (!student) notFound()
      ...
      <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
        <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">{student.classLevel as string}</p>
          </div>
        ...
        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5 px-6 py-6">
          <Field label="Admission number" value={(student.admissionNumber as string) || '—'} />
          <Field label="Class level" value={student.classLevel as string} />
          <Field label="Gender" value={student.gender as string} className="capitalize" />
          <Field
            label="Date of birth"
            value={`${formatDate(dob)} (${calculateAge(dob)} yrs)`}
          />
          <Field label="Parent / Guardian" value={(student.parentName as string) || '—'} />
          <Field label="Parent / Guardian phone" value={(student.parentPhone as string) || '—'} />
          <Field label="Enrollment date" value={formatDate(enrollmentDate)} />
          <Field
            label="Current balance"
            value={formatBalance(student.currentBalance as number)}
            className={(student.currentBalance as number) > 0 ? 'text-red-600 font-medium' : ''}
          />
        </dl>
      </div>
    </DashboardShell>
